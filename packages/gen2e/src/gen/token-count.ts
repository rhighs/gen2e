 ...
 */
export const fitsContext = (model: TiktokenModel, text: string): boolean => {
  return maxCharactersApprox(model) > text.length;
};
 ...
 */
export const maxCharactersApprox = (model: TiktokenModel): number => {
  const max = tokenLimits(model).context;
